...
class EVar(ABC):
    ...
    @staticmethod
    def has_value(str_rep: Optional[str]) -> bool:
        """Does the input string have at least one non-whitespace char?

        Args:
            str_rep: the variable value as read from the environment.
    ...
    def read(self, rep: Optional[str]):
        ...
        Returns:
            the parsed value if the string rep holds a value, the default
            value otherwise.
        """
...
class StrVar(EVar):
    """An env value parsed as a string.

    The string representation gets stripped of any leading and trailing
    whitespace.
    """
...
class BoolVar(EVar):
    """An env value parsed as a boolean.

    It evaluates to true just in case the string representation, after
    trimming any leading and trailing whitespace, is equal, ignoring case,
    to any of: 'true', 'yes', '1', 't'.
    """
...
class BitSizeVar(EVar):
    ...
    giga bytes, memory size in mega bytes, word size in bits, etc.

    This class is just a wrapper around the `bitmath` lib, see there for
    usage and examples.
    """
...
class EnvReader:
    ...
    @staticmethod
    def get_log_msg(var: EVar, value: Optional[str]) -> str:
        msgs = {
    ...
    def read(self, var: EVar):
        ...
        Returns:
            the parsed value if the variable is set, the variable's
            default otherwise.
        """
...
class YamlReader:
    ...
    def from_file(self, path: Optional[str], defaults: dict) -> dict:
        """Read the YAML in the specified file.

        Args:
            path: the path to the file.
            defaults: what to return if the path is empty.

        Returns:
            the YAML file as a dictionary if the path isn't empty,
    ...
    def from_env_file(self, env_var_name: str, defaults: dict) -> dict:
        ...
        Args:
            env_var_name: the env variable name. It's value is supposed
                to be a file path.
            defaults: what to return if the path is empty.
